...
int main (){
    freopen ("Go_Fish_Output.txt", "w", stdout);

    srand((unsigned)time(0));
    int delayLen = 0; //Set to 0 to save to text file.
    int player = 0;
    int numCards = 7;
    // const int WINSIZE = 14;     // amount of books needed to win
    const int FINSIZE = 26; // Sets when a game is finished.
    bool gameOn = true;
    ...
    // cin >> delayLen;
    // Comment out to save to text file.
    ...
    while (gameOn) {
        ...
        switch(player){
            //Player 1 Turn
            case 1:
                // system("clear"); // Comment out to save to text file.
                ...
                delay(delayLen);
                // Player will book any starting pairs they are dealt.
                while (p1.checkHandForBook(book1, book2)) {

                        p1.bookCards(book1, book2);
                        cout << "Starting book Found! " << p1.getName() << " books the " << book1 << " and " << book2 << endl;
                        delay(delayLen);
                ...
                if ((p2.getBookSize() + p1.getBookSize()) == FINSIZE) {
                    gameOn = false;
                }
                break;
            ...
            case 2:
                ...
                while (p2.checkHandForBook(book3, book4)) {
                        ...
                        cout << "Starting book Found! " << p2.getName() << " books the " << book3 << " and " << book4 << endl;
                        delay(delayLen);
    ...
    if (p1.getBookSize() > p2.getBookSize()){
      cout << p1.getName() << " is the winner with " << p1.getBookSize() << " books!" << endl;
    }
    else if (p1.getBookSize() < p2.getBookSize()){
      cout << p2.getName() << " is the winner with " << p2.getBookSize() << " books!" << endl;
    }
    else {
